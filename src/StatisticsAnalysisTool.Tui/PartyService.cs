...
public sealed class PartyService
{
    ...
    public void UpsertCharacter(long? objectId, Guid? guid, string? name, string? guildName, IReadOnlyList<int> equipment)
    {
        lock (_sync)
        {
            ...
            {
                member.Equipment = NormalizeEquipment(equipment);
            }
    ...
    {
        if (guid is null || guid == Guid.Empty || equipment.Count == 0)
        {
            return;
        }
        ...
        lock (_sync)
        {
            var member = GetOrCreateLocked(guid, null, null);
            member.Guid = guid;
            member.Equipment = NormalizeEquipment(equipment);
            member.LastUpdatedAt = DateTimeOffset.Now;
            IndexLocked(member);
    ...
    {
        if (guid is null || guid == Guid.Empty || itemPower is null or <= 0)
        {
            return;
        }
