...
public sealed class SessionTimelineService
{
    ...
    public void AddSilverCheckpoint(long silver, string? note)
    {
        lock (_sync)
        {
            var text = string.IsNullOrWhiteSpace(note)
                ? $"Silver checkpoint: {silver:N0}"
                : $"Silver checkpoint: {silver:N0} - {note.Trim()}";
    ...
    {
        var checkpoints = _events
            .Where(x => x.Silver is not null)
            .OrderBy(x => x.Time)
            .ToArray();
        ...
        var activeSeconds = new Dictionary<string, double>(StringComparer.OrdinalIgnoreCase);
        ...
        for (var i = 1; i < checkpoints.Length; i++)
        {
            ...
            var delta = current.Silver!.Value - previous.Silver!.Value;
            if (delta <= 0)
            {
                continue;
            }

            var participantSeconds = GetParticipantSecondsForIntervalLocked(previous.Time, current.Time);
            if (participantSeconds.Count == 0)
            {
                continue;
            }

            var totalSeconds = participantSeconds.Values.Sum();
            if (totalSeconds <= 0)
            {
                continue;
            }

            foreach (var (participant, seconds) in participantSeconds)
            {
                var share = (long)Math.Round(delta * (seconds / totalSeconds), MidpointRounding.AwayFromZero);
                splitSilver += share;
                shares[participant] = shares.GetValueOrDefault(participant) + share;
                activeSeconds[participant] = activeSeconds.GetValueOrDefault(participant) + seconds;
            }
            ...
            intervalList.Add(new LootSplitInterval(
                ...
                current.Time,
                delta,
                participantSeconds.Keys.OrderBy(x => x).ToArray(),
                delta / participantSeconds.Count));
        }
        ...
        return shares
            ...
            .ThenBy(x => x.Key)
            .Select(x => new LootSplitShare(x.Key, x.Value, activeSeconds.GetValueOrDefault(x.Key)))
            .ToArray();
    }
