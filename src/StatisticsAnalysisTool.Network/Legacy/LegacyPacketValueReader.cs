...
public static class LegacyPacketValueReader
{
    ...
    public static List<int> GetLegacyEquipment(IReadOnlyDictionary<byte, object> parameters, byte key)
    {
        var values = GetIndexedValues(parameters, key, ToInt);
        return values.Take(10).Select(x => Math.Max(0, x)).ToList();
    }

    public static List<long> GetLegacyLongSlots(IReadOnlyDictionary<byte, object> parameters, byte key)
    {
        return GetIndexedValues(parameters, key, ToLong);
    }
        ...
        where T : struct
    {
        ...
        if (value is IDictionary dictionary)
        {
            foreach (DictionaryEntry entry in dictionary)
            {
                ...
                if (convert(entry.Value) is { } typedValue)
                {
        ...
        if (value is IEnumerable enumerable and not string)
        {
            foreach (var item in enumerable)
            {
                values.Add(convert(item) ?? default);
            }
        }
